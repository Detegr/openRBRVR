...
/* TODO
static bool OpenVR::CreateVRRenderTarget(IDirect3DDevice9* dev, RenderTarget eye, uint32_t w, uint32_t h)
{
    ...
    , compositor(nullptr)
{
    ...
    try {
        ...
        openvrTexture[RightEye].handle = reinterpret_cast<void*>(&dxvkTexture[LeftEye]);
...
bool OpenVR::UpdateVRPoses(Quaternion* carQuat, Config::HorizonLock lockSetting)
{
    ...
    if (pose->bPoseIsValid) {
        HMDPose = glm::inverse(M4FromSteamVRMatrix(pose->mDeviceToAbsoluteTracking));
    }
    if (carQuat) {
        ...
        horizonLock = glm::identity<M4>();
    }
    return true;
}

FrameTimingInfo OpenVR::GetFrameTiming() const
{
